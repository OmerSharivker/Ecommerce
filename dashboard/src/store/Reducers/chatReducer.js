...
export const send_message_seller_admin = createAsyncThunk(
    ...
    async(info,{rejectWithValue, fulfillWithValue}) => {
        ...
        try {
            const {data} = await api.post(`/chat/admin/send-message-seller-admin`,info ,{withCredentials: true}) 
            // console.log(data)
            return fulfillWithValue(data)
        } catch (error) { 
            return rejectWithValue(error.response.data)
...
export const {messageClear,updateMessage,updateSellers,updateCustomers,updateAdminMessage,updateSellerMessage} = chatReducer.actions
