...
class dashboardController{
    ...
    get_admin_dashboard_data=async (req,res)=> {
        const {id}=req
            ...
            responseReturn(res,200,{totalProducts,totalOrders,totalSellers,messages,recentOrders,totalSales:totalSales.length> 0 ? totalSales[0].totalAmount : 0 ,})
            } catch (error) {
    ...
    get_seller_dashboard_data=async (req,res)=> {
        ...
        console.log(id)
            try {
                const totalSales = await sellerShopWallet.aggregate([
                    {
                        $match: {
                            sellerId: id
                        }
                ...
                const pendingOrders= await authOrder.find({
                    $and:[
                ...
                const totalProducts=await productModel.find({sellerId: new ObjectId(id)}).countDocuments()
                const totalOrders= await authOrder.find({sellerId: new ObjectId(id)}).countDocuments()
                ...
                const recentOrders=await await authOrder.find({sellerId: new ObjectId(id)}).limit(5)
            responseReturn(res,200,{totalProducts,totalOrders,pendingOrders,messages,recentOrders,totalSales:totalSales.length> 0 ? totalSales[0].totalAmount : 0 ,})
            } catch (error) {
                console.log(error.message)
            }
