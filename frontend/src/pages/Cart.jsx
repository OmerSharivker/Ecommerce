...
import { get_cart_product,delete_cart_product, messageClear,quantity_inc,quantity_dec } from '../store/reducers/cartReducer';
import toast from 'react-hot-toast';
const Cart = () => {

    const dispatch=useDispatch()
    const {userInfo} =useSelector(state => state.auth)
    const {cart_products,successMessage,shipping_fee,price,buy_product_item,outofstock_products} =useSelector(state => state.cart)
   ...
   useEffect (()=>{
    ...
    const inc =(quantity,stock,cart_id) => {
        ...
        if (temp <= stock) {
            ...
            toast.error(`This product out of stock only ${stock} left`)
        }
    }
...
<div className='flex justify-between w-5/12 sm:w-full sm:mt-3'>
  {
        pt.productInfo.discount > 0 ?  <div className='pl-4 sm:pl-0'>
        ...
        <button onClick={()=> (dispatch(delete_cart_product(pt._id)))} className='px-5 py-[3px] bg-red-500 text-white'>Delete</button>
             ...
             } 
             {
               ...
               <div className='bg-white p-4'>
               {
                    ...
                    <span className='text-sm'>{p.products[0].brand}</span>
                    </div>
                        </div>
                    </div>
<div className='flex justify-between w-5/12 sm:w-full sm:mt-3'>
{
        p.products[0].discount > 0 ?  <div className='pl-4 sm:pl-0'>
